        ...
        if (not kwargs):
            ...
            for name, field_type, is_required in self._fields:
                if is_required and name not in kwargs.keys():
                    raise TypeError(
                        "Key: {} is required but not found in the json data".format(name))
                v = kwargs.pop(name, {})
...
class XiaoAIUser(XiaoAIBase):
    _fields = [('user_id', str, True), ('access_token', str, False), ('is_user_login',
                                                                      bool, False), ('gender', str, False), ('real_user_id', str, False)]


class XiaoAISession(XiaoAIBase):
    _fields = [('is_new', bool, True), ('session_id', str, True), ('application',
                                                                   XiaoAIApplication, True), ('attributes', dict, False), ('user', XiaoAIUser, False)]
...
class XiaoAIContext(XiaoAIBase):
    _fields = [('passport', dict, False), ('device_id', str, False),
               ('bind_id', str, False), ('app_info', list, False)]
...
class XiaoAISlot(XiaoAIBase):
    _fields = [('name', str, True), ('value', str, False),
               ('is_inquire_failed', bool, False)]


class XiaoAISlotInfo(XiaoAIBase):
    _fields = [('intent_name', str, True), ('is_confirmed',
                                            bool, False), ('slots', list, False)]
...
class XiaoAIRequest(XiaoAIBase):
    _fields = [('request_id', str, True), ('timestamp', int, True), ('locale', str, True), ('intent', XiaoAIIntent, False), ('no_response', bool, False),
               ('type', int, True), ('slot_info', XiaoAISlotInfo, False), ('event_type', str, False), ('event_property', XiaoAIEventProperty, False)]


class XiaoAIOpenRequest(XiaoAIBase):
    _fields = [('version', str, True), ('query', str, False), ('session', XiaoAISession,
                                                               True), ('context', XiaoAIContext, False), ('request', XiaoAIRequest, True)]
...
class XiaoAIToDisplay:
    def __init__(self, type_=None, url=None, text=None, ui_template=None, ui_type=None, phone_template=None):
        ...
        if (phone_template is not None and type(phone_template) is not XiaoAIPhoneTemplate):
            raise TypeError(
                "ToDisplay.phone_template must be XiaoAIPhoneTemplate type")
...
class XiaoAIPushAppActionProperty:
    def __init__(self, title=None, content=None, uri=None):
        if (title is not None and type(title) is not str):
            raise TypeError(
                "XiaoAIPushAppActionProperty.title must be str type")
        self.title = title

        if (content is not None and type(content) is not str):
            raise TypeError(
                "XiaoAIPushAppActionProperty.content must be list type")
...
class XiaoAIDeviceCommandActionProperty:
    def __init__(self, command=None):
        if (command is not None and type(command) is not str):
            raise TypeError(
                "XiaoAIDeviceCommandActionProperty.command must be str type")
...
class XiaoAIQuickAppCardActionProperty:
    def __init__(self, card_params):
        if (card_params is not None and type(card_params) is not dict):
            raise TypeError(
                "XiaoAIQuickAppCardActionProperty.card_params must be dict type")
...
class XiaoAIIntentInfoExtra:
    def __init__(self, intent_type=None, uri=None):
        if (intent_type is not None and type(intent_type) is not str):
            raise TypeError(
                "XiaoAIIntentInfoExtra.intent_type must be str type")
...
class XiaoAIActionProperty:
    def __init__(self, file_id_list=None, quick_app_path=None, push_app_info=None, quick_app_extra=None, device_command=None, app_intent_info=None, quick_app_card_info=None):
        ...
        if (push_app_info is not None and type(push_app_info) is not XiaoAIPushAppActionProperty):
            raise TypeError(
                "ActionProperty.push_app_info must be XiaoAIPushAppActionProperty type")
        ...
        if (device_command is not None and type(device_command) is not XiaoAIDeviceCommandActionProperty):
            raise TypeError(
                "ActionProperty.device_command must be XiaoAIDeviceCommandActionProperty type")
        self.device_command = device_command

        if (app_intent_info is not None and type(app_intent_info) is not XiaoAIIntentInfoExtra):
            raise TypeError(
                "ActionProperty.app_intent_info must be XiaoAIIntentInfoExtra type")
        ...
        if (quick_app_card_info is not None and type(quick_app_card_info) is not XiaoAIQuickAppCardActionProperty):
            raise TypeError(
                "ActionProperty.quick_app_card_info must be XiaoAIQuickAppCardActionProperty type")
...
class XiaoAIResponse:
    def __init__(self, to_speak=None,
        ...
        if (action_property is not None and type(action_property) is not XiaoAIActionProperty):
            raise TypeError(
                "Response.action_property must be ActionProperty type")
        ...
        if (is_directive_not_interrupted is not None and type(is_directive_not_interrupted) is not bool):
            raise TypeError(
                "Response.is_directive_not_interrupted must be bool type")
